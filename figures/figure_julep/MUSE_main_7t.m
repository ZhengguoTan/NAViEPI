...
filename=[filepath1, 'julep_k.h5'];
sensname=[filepath1, 'julep_c.h5'];
saveimg=[savepath, 'img_MB3_muse_7t.mat'];
...
for sl=1:par.nSL
    ...
    acs_y=min([50, 2*par.ky_s-par.ky_r, size(b1ksp2, 2)]);% NAV size (default): 64
    acs_x=acs_y*MB;
...
%     phi=2*pi*2/MB;% or -2*pi/MB depending on CAIPI implementation
